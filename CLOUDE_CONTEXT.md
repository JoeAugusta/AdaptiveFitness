...
Supabase tables: users, user_profiles, goals, plans, workout_logs, macro_plans
Config files: Lib/supabase.ts, Lib/Revenuecat.ts
Edge Functions: supabase/functions/generate-plan, supabase/functions/coaching-feedback
...
## CURRENT PHASE
Phase 2 — Core Loop
Phase 1 complete and all screens building successfully.
Next screens: S12 Exercise Library, S13 Progress Charts, S15 Weekly Coach Summary,
S16 Macro Tracker, and the adaptive coaching engine (weekly plan updates).
...
- Called from: S15 WeeklyCoachSummaryScreen via supabase.functions.invoke
- Purpose: Reads workout_logs + plan_json, computes performance metrics, calls Claude API to generate a structured weekly debrief
- JWT verification: DISABLED
- Secrets: ANTHROPIC_API_KEY, SUPABASE_SERVICE_ROLE_KEY (auto-available in edge functions)
- Receives: { userId: string, planId: string, weekNumber: number }
- Returns: { summary: { weekNumber, performanceRating, headline, highlights, performanceSummary, nextWeekChanges, nutritionCheckin, motivationalNote } }
...
- Both Edge Functions have JWT verification DISABLED in Supabase dashboard
...
- Use Sonnet 4.6 for simpler screens and fixes
...
1. S15 Weekly Coach Summary — generates weekly debrief via Claude API
2. Adaptive coaching engine — generates Week 2+ based on workout_logs
