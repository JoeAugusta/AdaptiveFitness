...
// Optional: ONLY_PROFILE=sim_1 RPE_SCENARIO=easy node audit-progression.mjs
...
const REAL_USER_ID = 'd0338c58-23d1-4aaf-8793-ae61d40f5ee2';

const RPE_BY_SCENARIO = { easy: 6, ontarget: 7, hard: 9 };
const ALL_SCENARIOS = ['easy', 'hard', 'ontarget'];
...
function lookupW1Weight(exerciseName, w1Weights) {
  ...
  for (const [key, val] of Object.entries(w1Weights)) {
    ...
    if (lower.includes(keyLower) || keyLower.includes(lower)) {
      console.warn(`lookupW1Weight fuzzy: "${exerciseName}" -> "${key}"`);
      return val;
    }
  }
  console.error(`lookupW1Weight: no match for "${exerciseName}"`);
  return null;
}

function buildSetsJson(exercises, rpe, w1Weights, isWeek1) {
  const sets = [];
...
const COMPOUND_NAME_RE = /\b(squat|deadlift|bench|overhead press|row|press)\b/i;
...
function isCompoundKey(key) {
  return COMPOUND_NAME_RE.test(exerciseNameFromKey(key));
}

function matchesTargetLift(exerciseName, targetLift) {
  const normalized = targetLift.replace(/_/g, ' ').toLowerCase();
...
function weekHasTargetLift(weights, targetLift) {
  return Object.keys(weights).some(key =>
    matchesTargetLift(exerciseNameFromKey(key), targetLift),
  );
}

function runAssertions(profile, scenario, weeklyWeights, weeklyPhases) {
  const failures = [];

  // CLONE-DRIFT: W1 keys must appear in W2, W3, W4
  const w1Keys = Object.keys(weeklyWeights[0] ?? {});
  for (let w = 1; w < weeklyWeights.length; w++) {
    const wnKeys = new Set(Object.keys(weeklyWeights[w] ?? {}));
    for (const key of w1Keys) {
      if (!wnKeys.has(key)) {
        failures.push(`DRIFT: '${key}' present W1 but missing W${w + 1}`);
      }
    }
  }

  // TARGET LIFT: must appear in every week W1-W4
  if (profile.payload.targetLift) {
    for (let w = 0; w < weeklyWeights.length; w++) {
      if (!weekHasTargetLift(weeklyWeights[w] ?? {}, profile.payload.targetLift)) {
        failures.push(
          `CRITICAL: target lift '${profile.payload.targetLift}' missing in W${w + 1}`,
        );
      }
    }
  }

  // PHASE: W2-W4 must not be baseline or undefined
  for (const { weekNumber, phase } of weeklyPhases) {
    if (weekNumber >= 2 && (!phase || phase === 'baseline')) {
      failures.push(`PHASE: W${weekNumber} phase is '${phase ?? 'undefined'}'`);
    }
  }

  // DIRECTION: scenario-specific compound W2 vs W1
  const w1 = weeklyWeights[0] ?? {};
  const w2 = weeklyWeights[1] ?? {};
  for (const key of Object.keys(w1)) {
    if (!isCompoundKey(key)) continue;
    const w1Weight = w1[key];
    const w2Weight = w2[key];
    if (w1Weight == null || w2Weight == null) continue;
    if (w1Weight === 0 || w2Weight === 0) continue;

    if (scenario === 'easy' && w2Weight < w1Weight) {
      failures.push(`EASY-DROP: '${key}' W1=${w1Weight} W2=${w2Weight}`);
    }
    if (scenario === 'hard' && w2Weight > w1Weight * 1.05) {
      failures.push(`HARD-JUMP: '${key}' W1=${w1Weight} W2=${w2Weight}`);
    }
  }
...
function printPassFail(profile, scenario, failures) {
  if (failures.length === 0) {
    console.log(`  ✅ PASS (${profile.label} / ${scenario})`);
  } else {
    console.log(`  ❌ FAIL (${profile.label} / ${scenario}) — ${failures.length} issue(s):`);
...
async function simulateProfile(profile, scenario) {
  const rpe = RPE_BY_SCENARIO[scenario];
  const cleanupIds = { planId: null, goalId: null, logIds: [] };

  try {
    console.log(`\n${'═'.repeat(72)}`);
    console.log(`SIMULATING: ${profile.label} / ${scenario} (RPE ${rpe})`);
    console.log('═'.repeat(72));
    ...
    for (let weekNum = 1; weekNum <= 4; weekNum++) {
      ...
      weeklyPhases.push({ weekNumber: weekNum, phase: week?.phase ?? null });
      ...
      console.log(`    Logging ${workoutDays.length} sessions (RPE ${rpe}, week ${weekNum === 1 ? 'pre-populated W1 weights' : 'plan weights'})...`);

      for (const day of workoutDays) {
        const isWeek1 = weekNum === 1;
        const setsJson = buildSetsJson(day.exercises ?? [], rpe, profile.w1Weights, isWeek1);
    ...
    printProgressionTable(`${profile.label} / ${scenario}`, weeklyWeights);

    const failures = runAssertions(profile, scenario, weeklyWeights, weeklyPhases);
    printPassFail(profile, scenario, failures);
...
async function run() {
  ...
  const scenarioEnv = process.env.RPE_SCENARIO;
  const profiles = only ? PROFILES.filter(p => p.id === only) : PROFILES;

  if (only && profiles.length === 0) {
    console.error(`No profile id "${only}"`);
    process.exit(1);
  }

  const scenarios = scenarioEnv ? [scenarioEnv] : ALL_SCENARIOS;
  if (scenarioEnv && !RPE_BY_SCENARIO[scenarioEnv]) {
    console.error(`Invalid RPE_SCENARIO "${scenarioEnv}" — use easy, hard, or ontarget`);
    process.exit(1);
  }

  console.log('Hone — 4-Week Progression Simulation (pre-populated W1 weights)');
  console.log(`Profiles: ${profiles.map(p => p.id).join(', ')} | Scenarios: ${scenarios.join(', ')}\n`);

  const results = [];
  for (const profile of profiles) {
    for (const scenario of scenarios) {
      const result = await simulateProfile(profile, scenario);
      results.push(result);
  ...
  const failed = results.filter(r => r.status === 'ok' && !r.passed).length;
  console.log(`Summary: ${ok} ok (${passed} passed, ${failed} failed assertions), ${errors} errors`);
}
